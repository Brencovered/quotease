...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: matRows }, { data: teamRows }, { data: taskRows }, boardColumns, timesheetEntries, { data: lineItems }] = await Promise.all([
        ...
        <div className="card mb-4">
          ...
          {scopeLines.length > 0 ? (
            <ul className="grid sm:grid-cols-2 gap-y-1 gap-x-4 mb-4">
              {scopeLines.map((line) => (
                <li key={line} className="text-[13.5px] text-[var(--ink-soft)]">&bull; {line}</li>
              ))}
            </ul>
          ) : (
