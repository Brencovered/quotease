...
function calcItemTotal(item: PackageItem): number {
...
export default function PackagesPage() {
  ...
  async function loadPackages(bid: string) {
    ...
    const transformed: Package[] = (data ?? []).map((row: PackageRow) => ({
      ...row,
      package_items: row.package_items ?? [],
      use_count: 0, /* placeholder - can be enhanced later */
    }));
  ...
  function addItemRow() {
    setFormItems((prev) => [
      ...prev,
      { ...EMPTY_ITEM, sort_order: prev.length },
    ]);
  }
  ...
  async function handleSave() {
    ...
    const validItems = formItems.filter(
      (item) => item.label.trim() && item.qty > 0 && item.unit_cost >= 0
    );
  ...
  const mostUsedPackage =
    packages.length > 0
      ? packages.reduce((prev, curr) =>
          (curr.use_count ?? 0) > (prev.use_count ?? 0) ? curr : prev
        )
      : null;
      ...
      {!loading && packages.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {packages.map((pkg) => {
            ...
            return (
              <div key={pkg.id} className="card flex flex-col gap-3 reveal" style={{ animationDelay: `${Math.min(packages.indexOf(pkg) * 0.05, 0.3)}s` }}>
                {/* Card header */}
                ...
                {/* Description */}
                {pkg.description && (
                  <p className="text-[12px] text-[var(--ink-soft)] leading-relaxed line-clamp-2">
                    {pkg.description}
                  </p>
                )}
                ...
                {itemCount > 0 && (
                  <div className="bg-[var(--app-bg)] rounded-lg px-3 py-2.5 space-y-1">
                    ...
                    {itemCount > 3 && (
                      <div className="text-[11px] text-[var(--ink-faint)] pt-0.5">
                  ...
                  {formItems.map((item, idx) => (
                    ...
                    >
                      <input
                        ...
                        value={item.label}
                        onChange={(e) =>
                          updateItemRow(idx, { label: e.target.value })
                        }
