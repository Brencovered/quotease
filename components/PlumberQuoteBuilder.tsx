...
export default function PlumberQuoteBuilder({ profile, materials }: {
  ...
  async function saveAndSend(sendEmail: boolean) {
    ...
    const { data: quote, error } = await supabase.from("quotes").insert({ profile_id: user.id, client_name: clientName, client_email: clientEmail, site_address: siteAddress, trade: "plumber", job_type: intake.jobType, intake_data: intake, labour_hours: result.labourHours, materials_cost: result.materialsCost, total_cost: result.totalCost, payment_terms: paymentTerms, status: sendEmail ? "sent" : "draft" }).select().single();
