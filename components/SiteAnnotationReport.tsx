          ...
          <div className="bg-[var(--app-bg)] rounded-xl border border-[var(--line)] overflow-hidden mb-2">
            {anns.map((ann, i) => (
              <div key={ann.id} className={`flex items-center gap-2 px-3 py-2 text-[12.5px] ${i < anns.length - 1 ? "border-b border-[var(--line-subtle)]" : ""}`}>
                ...
                {ann.note && <span className="text-[var(--ink-faint)] truncate max-w-[80px]">{ann.note}</span>}
              </div>
            ))}
          </div>
                ...
                <div key={ann.id} className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl overflow-hidden">
                  {ann.frameData && (
                    <div className="relative">
                      ...
                      <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-black/60 rounded-full px-3 py-1.5">
                        ...
                        <span className="text-white text-[11px] font-bold">#{idx}</span>
                      </div>
                      {/* Room badge on photo */}
                  ...
                  <div className="p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-bold text-[13.5px] text-[var(--ink)]">{ann.label}</p>
